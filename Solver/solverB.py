...
def solverB(id, mh, maxIter, pop, function, lb, ub, dim):
    ...
    if mh == 'GO':
       poblacion = lb + (ub - lb) * np.random.rand(pop, dim)
    ...
    print("iteracion: "+
            ...
            ", DIV: "+str(div_t))
    results.write(
        f'0,{str(BestFitness)},{str(round(tiempoInicializacion2-tiempoInicializacion1,3))},{str(XPL)},{str(XPT)},{str(div_t)}\n'
    )
    ...
    for iter in range(0, maxIter):
        ...
        if mh == 'GO':
            ...
            xmin = lb[0]
            ben = True
            poblacion = GO(pop, dim, xmax, xmin, maxIter, function, iter, np.array(poblacion.tolist()), fitness, ben)
        if mh == "SCA":
